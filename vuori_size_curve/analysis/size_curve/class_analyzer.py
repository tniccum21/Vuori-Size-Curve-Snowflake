"""
Class-level size curve analyzer.
"""
...
class ClassAnalyzer(BaseAnalyzer):
    """
    Analyzer for class-level size curves.
    """
    ...
    def analyze(
        ...
        **kwargs
        ...
        """
        Analyze sales data to generate class-level size curves.
        ...
        Returns:
            Dict[str, SizeCurve]: Dictionary mapping class identifiers to size curves
        """
        result = {}
        
        # Get the class column name
        level_column = LEVEL_COLUMN_MAP.get('class')
        if level_column not in sales_data.columns:
            ...
            class_candidates = [col for col in sales_data.columns if 'class' in col.lower() and 'sub' not in col.lower()]
            if class_candidates:
                level_column = class_candidates[0]
                print(f"Using alternative column for class: {level_column}")
            else:
                return result
        
        # Filter out invalid size data and _N/A classes
        ...
        for (class_value, gender_code), class_group in filtered_sales_data.groupby([level_column, 'GENDER_CODE']):
            # Skip groups with insufficient sales
            if class_group['TOTAL_SALES'].sum() < self.min_sales:
                continue
            
            # Further group by template from the product data
            for template_name, template_group in class_group.groupby('TEMPLATE'):
                ...
                # Calculate total sales for this class, gender, and template
                ...
                if total_template_sales > 0:
                    ...
                    composite_key = f"{class_value}|{gender_code}|{template_name}"
                    
                    # Create a SizeCurve object
                    size_curve = SizeCurve(
                        item_code=class_value,
        ...
        print(f"Generated {len(result)} class-level size curves.")
        return result